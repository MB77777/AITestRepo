...
public abstract class AGUIAbstractLangGraphAgent implements LG4JLoggable {
    ...
    protected abstract <S extends AgentState> List<Approval> onInterruption(
    ...
    protected Collection<? extends BaseEvent> nodeOutputToEvents(
            RunAgentParameters input,
    ...
    public final Flux<? extends BaseEvent> run(RunAgentParameters input) {
        final var graphData = graphByThread.computeIfAbsent(
                input.getThreadId(),
                TryFunction.Try(ignored -> buildStateGraph())
        );
            ...
            var outputFlux = Flux.<BaseEvent>create(emitter -> {
                for (var event : outputGenerator) {
                    if (event instanceof StreamingOutput<? extends AgentState> output) {
                        ...
                        if (messageId == null) {
                            messageId = streamingId.updateAndGet(ignored -> newMessageId());
